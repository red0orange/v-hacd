...
#define TEST_FOR_MEMORY_LEAKS                                                  \
  0 // set to 1, on Windows only, to enable memory leak checking on application
    // exit
...
bool getTrueFalse(const char *option, bool &value) {
  ...
  if (strcmp(option, "t") == 0 || strcmp(option, "true") == 0 ||
      strcmp(option, "1") == 0) {
    ret = true;
    value = true;
  } else if (strcmp(option, "f") == 0 || strcmp(option, "false") == 0 ||
             strcmp(option, "0") == 0) {
    ret = true;
    value = false;
  } else {
    printf("Valid values are 'true' or 'false', 't' or 'f', or '1' or '0', "
           "only.\n");
  }
...
public:
  ...
  // Be aware that if you are running V-HACD asynchronously (in a background
  // thread) this callback will come from a different thread. So if your
  // print/logging code isn't thread safe, take that into account.
  virtual void Update(const double overallProgress, const double stageProgress,
                      const char *const stage, const char *operation) final {
    char scratch[512];
    snprintf(scratch, sizeof(scratch), "[%-40s] : %0.0f%% : %0.0f%% : %s",
             stage, overallProgress, stageProgress, operation);
  ...
  // This is an optional user callback which is only called when running V-HACD
  // asynchronously. This is a callback performed to notify the user that the
  // convex decomposition background process is completed. This call back will
  // occur from a different thread so the user should take that into account.
  virtual void NotifyVHACDComplete(void) { Log("VHACD::Complete"); }
...
enum class ExportFormat { NONE, WAVEFRONT, STL };
...
int main(int argc, const char **argv) {
  ...
  if (argc < 2) {
    ...
    printf("\n");
    printf("-h <n>                  : Maximum number of output convex hulls. "
           "Default is 32\n");
    printf("-r <voxelresolution>    : Total number of voxels to use. Default "
           "is 100,000\n");
    printf("-e <volumeErrorPercent> : Volume error allowed as a percentage. "
           "Default is 1%%\n");
    printf("-d <maxRecursionDepth>  : Maximum recursion depth. Default value "
           "is 12.\n");
    printf("-s <true/false>         : Whether or not to shrinkwrap output to "
           "source mesh. Default is true.\n");
    printf("-f <fillMode>           : Fill mode. Default is 'flood', also "
           "'surface' and 'raycast' are valid.\n");
    printf("-v <maxHullVertCount>   : Maximum number of vertices in the output "
           "convex hull. Default value is 64\n");
    printf("-a <true/false>         : Whether or not to run asynchronously. "
           "Default is 'true'\n");
    printf("-l <minEdgeLength>      : Minimum size of a voxel edge. Default "
           "value is 2 voxels.\n");
    printf("-p <true/false>         : If false, splits hulls in the middle. If "
           "true, tries to find optimal split plane location. False by "
           "default.\n");
    printf("-o <output_path>            : Export the convex hulls as a series "
           "of wavefront OBJ files or STL files.\n");
    printf("-l <true/false>         : If set to false, no logging will be "
           "displayed.\n");
  } else {
    ...
    if (tcount == 0) {
      ...
      for (int i = 2; i < argc; i += 2) {
        ...
        if (strcmp(option, "-h") == 0) {
          ...
          if (v < 1 || v > 100000) {
            printf("Invalid maximum number of convex hulls. Valid range is "
                   "1-100000\n");
          } else {
          ...
          if (r >= 10000 && r <= 10000000) {
            ...
            printf("Invalid voxel resolution must be between 10,000 and "
                   "10,000,000\n");
          }
          ...
          //     printf("Saving the output convex hulls as a series of wavefront
          //     "
          //            "OBJ files.\n");
          //   } else if (strcmp(value, "stl") == 0) {
          //     format = ExportFormat::STL;
          //     printf(
          //         "Saving the output convex hulls as a single ASCII STL
          //         file.\n");
          //   } else {
          //     printf("Unknown export file format (%s). Currently only support
          //     "
          //            "'obj' and 'stl'\n",
          //            value);
          //   }
          ...
          if (r >= 2 && r <= 64) {
            ...
            printf(
                "Invalid maximum recursion depth, must be between 2 and 64\n");
          }
          ...
          if (strcmp(value, "flood") == 0) {
            ...
            printf("Invalid fill mode, only valid options are 'flood', "
                   "'raycast', and 'surface'\n");
          }
          ...
          if (r >= 8 && r <= 2048) {
            ...
            printf(
                "Invalid maximum hull vertices, must be between 8 and 20484\n");
          }
          ...
          if (r >= 1 && r <= 32) {
            ...
            printf("Invalid minimum voxel edge length, must be between 1 and "
                   "32\n");
          }
        }
      }
...
#endif
#ifdef _MSC_VER
      printf("Press the SPACEBAR to cancel convex decomposition before it has "
             "completed.\n");
#endif
      ...
      if (!canceled && iface->GetNConvexHulls()) {
        ...
        snprintf(outputName, sizeof(outputName), "%s_decompose.stl",
                 baseName.c_str());
        ...
        if (dot && format != ExportFormat::NONE) {
          if (format == ExportFormat::STL) {
            for (uint32_t i = 0; i < iface->GetNConvexHulls(); i++) {
              ...
              char hullName[2048];
              snprintf(hullName, sizeof(hullName), "%s%03d.stl",
                       baseName.c_str(), i);
              ...
              if (fph) {
                ...
                for (uint32_t j = 0; j < ch.m_triangles.size(); j++) {
                  ...
                  double normal[3];
                  FLOAT_MATH::fm_computePlane((double *)&p1, (double *)&p2,
                                              (double *)&p3, normal);
            ...
            for (uint32_t i = 0; i < iface->GetNConvexHulls(); i++) {
              ...
              iface->GetConvexHull(i, ch);
              char outputName[2048];
              snprintf(outputName, sizeof(outputName), "%s%03d.obj",
                       baseName.c_str(), i);
        ...
        if (!output_path.empty()) {
          ...
          FILE *fph = fopen(output_path.c_str(), "wb");
          if (fph) {
            // printf("Saving Convex Decomposition results of %d convex hulls
            // to"
            //        "'decomp.obj'\n",
        ...
        //   if (fph) {
        //     printf("Saving Convex Decomposition results of %d convex hulls to
        //     "
        //            "'decomp.obj'\n",
        //            iface->GetNConvexHulls());
        //     uint32_t baseIndex = 1;
        ...
        //   if (fph) {
        //     printf(
        //         "Saving convex hull results to a single file
        //         'decomp.stl'\n");
        //     for (uint32_t i = 0; i < iface->GetNConvexHulls(); i++) {
        //       VHACD::IVHACD::ConvexHull ch;
        //       iface->GetConvexHull(i, ch);
        ...
        //       if (fph) {
        //         char hullName[2048];
        //         snprintf(hullName, sizeof(hullName), "%s%03d",
        //         baseName.c_str(),
        //                  i);
        //         fprintf(fph, "solid %s\n", hullName);
        //         for (uint32_t j = 0; j < ch.m_triangles.size(); j++) {
